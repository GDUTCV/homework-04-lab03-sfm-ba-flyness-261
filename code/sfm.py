...
def get_reprojection_residuals(points2d: np.ndarray, points3d: np.ndarray, intrinsics: np.ndarray,
    ...
    homo_3d_points = np.hstack((points3d, np.ones((points3d.shape[0], 1))))

    extrinsics = np.hstack((rotation_mtx, tvec.reshape(-1, 1)))
    P = intrinsics @ extrinsics

    projected_points_homogeneous = P @ homo_3d_points.T
...
def solve_pnp(image_id: str, point2d_idxs: np.ndarray, all_points3d: np.ndarray, point3d_idxs: np.ndarray,
    ...
    for _ in range(num_ransac_iterations):
        ...
        _, rotation_vector, tvec = cv2.solvePnP(selected_pts3d, selected_pts2d, intrinsics, distCoeffs=None, flags=cv2.SOLVEPNP_ITERATIVE)
