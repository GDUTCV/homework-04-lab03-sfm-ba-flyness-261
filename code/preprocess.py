...
def create_ransac_matches(image_file1: os.path, image_file2: os.path,
    ...
    """
    essential_mtx, inliers = cv2.findEssentialMat (points1, points2, camera_intrinsics, cv2.RANSAC, ransac_threshold, prob=0.999 )
...
def create_scene_graph(image_files: list, min_num_inliers: int = 40):
    ...
    for i, j in combinations(range(number_images), 2):
